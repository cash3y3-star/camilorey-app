...
export async function getServerSideProps({ query }) {
  ...
  async function buildFormAndH2H(pairs) {
    ...
    // 504 en producción (la función tardaba más de 60s). Ahora se
    // trae en LOTES (mismo patrón que ya usaba el H2H de abajo): un OR
    // de hasta 20 jugadores por consulta, límite generoso por lote, y
    // se agrupa/recorta a 20 por jugador acá. Un mismo partido puede
    // volver en más de un lote (si sus dos jugadores cayeron en lotes
    // distintos) — se dedupea por id de partido antes de recortar.
    ...
    <div className={`pick-card ${featured ? 'pick-card-featured' : ''}`} onClick={onClick}>
      ...
      <div className="pc-head">
        {featured ? (
          ...
          <span className="tier-badge tier-exclusive">
...
// Barras de forma reciente (una por partido, verde=ganó/rojo=perdió) —
// adaptado de una referencia guardada que usaba altura variable para
// un stat numérico (runs, etc). Acá el resultado es binario
...
const CSS = `
  ...
  *{box-sizing:border-box;}
  ...
  .form-bar.miss{background:var(--miss);}
  .form-bar-label{font-size:10px; font-weight:800; color:rgba(0,0,0,.55);}
