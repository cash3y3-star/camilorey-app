 ...
 */
function computeConfidence({ ratingDiff, streakA, streakB, h2hWinsA, h2hTotal, h2hLastResult = null }) {
  ...
  // aunque el orden sí sea predecible. Este factor mira solo quién
  // ganó el cruce MÁS RECIENTE: si A lo perdió, favorece que A gane
  // ahora (y viceversa). Peso moderado a propósito — es un factor
  // nuevo sin medir todavía contra resultados reales (ver
  // /api/model-stats una vez se acumulen picks resueltos con esto).
  const altScore = h2hLastResult === 'A' ? -1 : h2hLastResult === 'B' ? 1 : 0;
