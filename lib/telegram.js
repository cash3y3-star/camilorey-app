...
// "Ruzicka J — Erik Marres / Ganador 🥇Erick Mares / cuota 1.64",
// con el encabezado (qué tipo de aviso es) arriba.
function buildPickCaption({ label, favName, rivalName, odds }) {
  const lines = [label, `${favName} — ${rivalName}`, `Ganador 🥇${favName}`];
  if (odds) lines.push(`cuota ${Number(odds).toFixed(2)}`);
  return lines.join('\n');
}
