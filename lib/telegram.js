// ============================================================
// CZECH IA AGENTS — mandar mensajes por Telegram (chat personal del admin,
...
// overrideIds (opcional) — pedido 2026-07-27: cuando un tipster que
// NO es CAMILO REY destaca un pick, el aviso tiene que ir SOLO a su
// propio canal (profiles.telegram_chat_id), no a la lista global de
// TELEGRAM_CHAT_ID. Sin override, se comporta igual que siempre.
async function sendTelegramMessage(text, overrideIds) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  const ids = overrideIds && overrideIds.length ? overrideIds : chatIds();
...
async function sendTelegramPhoto(photoUrl, caption, overrideIds) {
  if (!photoUrl) return sendTelegramMessage(caption, overrideIds);
